...
print(add_sub(5, 3))    # 튜플 하나를 리턴  (기존 0, 1개만을 리턴한다는 원칙에 위배되지 않음)
print(*add_sub(5, 3))
...
# 디폴트 매개변수는 항상 일반 매개변수 뒤에 위치해야 한다.     def sub(a = 0, b): error

def sub(a, b):
    return a - b

print(sub(5, 10))
# print(sub(5))     # 디폴트 매개변수 때문에 가능 def sub(a, b = 0)
# print(sub())    # a = 0, b = 0 
...
print(sub(5, 10))              # 순서대로 매칭
print(sub(b = 10, a = 5))      # 키워드 인자로 매칭

def introduce(name, city, mbti = "CUTE"):
    return f"저는 {name}이고, {city}에 살고 {mbti}입니다."

print(introduce("뽀로로", "일산"))
print(introduce(mbti = "ENTJ", name = "임세진", city = "수원"))
...
def add_all(*args):    # 여기의 *은 묶는 별 ------------- 아래의 별은 푸는 별
    print(args)          # 튜플로 만든다음 -> sum 함
    return sum(args)
...
a = [1, 2, 3]
print(add_all(*a))   # 언패킹해서 넣어줘야 함 (=풀어 넣어줘야 함)
...
def introduce2(**kwargs):  # ** -----> 딕셔너리로 만듦(= 묶는거) --- 아래와 다름
    print(kwargs)
...
def pocket_money(rm, *args, **kwargs):
    tot = rm
    tot += sum(args)
    tot += sum(kwargs.values())
    return tot

print(pocket_money(500, 100, 200, dad = 10000, mother = 5000))   # key 값, value 값 둘다 써야함
print(pocket_money(500, 100, 200, 300, dad = 10000, mother = 5000, uncle = 100000))