...
d.update(d2)            # upsert: update + insert
print(d)
...
d2 = {"eng": 90, "sci": 95}
print({**d, **d2})        # update 메소드와 동일(새로운 객체 생성)
# print({**d} | {**d2})   나 개인의 것
...
result = {k: v for v, k in result.items()}
print(result)

# 점수 90 이상만 필터링하기
scores = {"국어": 90, "영어": 95, "수학": 100, "과학": 35}
...
#3. Counter: 요소의 빈도수를 자동으로 세우주는 딕셔너리 서브클래스
...
# 2. dictionary comprehension
stock.update({k: stock.get(k, 0) + v for k, v in incoming.items()})
print(stock)
