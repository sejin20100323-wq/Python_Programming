...
langs.sort()                        # 오름차순 정렬   // 원본을 오름차순함(!= sorted)
print(langs)

langs.sort(reverse = True)            # 내림차순 정렬
print(langs)

langs.clear                       # 모든 item 삭제  (!= remove  :  한개만 지움)
print(langs)

# 리스트 복사
ori = [1, 2, 3]

result = ori.copy()   # 사본을 만듦
result.append(10)
...
a, b, c = zip(subjects, scores)     # strict=True 를 사용하면 subjects 리스트와 scores가 매칭이 안되면 error를 냄 *원래는 그냥 무시함
print(a, b, c)

for subject, score in zip(subjects, scores):
    print(f"{subject}: {score}점")

# [x]는 x 자체를 원소 하나로 해서 리스트에 넣는다
print([zip(subjects, scores)])

# list(x)는 x를 순회해서 리스트에 넣음
print(list(zip(subjects, scores)))

print(["python"])        # 얘는 자체를 넣음
print(list("python"))    # 얘는 하나씩 뽑아서 넣음
...
# print(result)                              이것도 가능은 함(이 실습은 if문 연습하려고 넣은 실습이기에)

# 1 ~ 10 중 짝수면 "짝", 홀수면 "홀" 출력하기
result = ["짝" if x % 2 == 0 else "홀" for x in range(1, 11)]    # 삼항 연산자는 for문 앞에 쓴다
print(result)
...
result = sum([i for i in range(1, 101) if i % 3 == 0 or i % 5 == 0])     # if not i%3 == 0 or not i%5 == 0도 가능
...
result = [x for x in words if x.find("n") >= 0]     # 0보다 크거나 같으면 있는 거임 
result = [x for x in words if x.count("n")]         # count여서 그냥 써도 됨
...
result = [sum(score) / len(score) for score in zip(std1, std2, std3)]         # 튜플
result = [sum(score) / len(score) for score in zip(*scores)]         # 언패킹(조금 더 쉬움)
